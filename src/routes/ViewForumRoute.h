...
{
    // View forum
    CROW_ROUTE(app, "/forum/<int>")
        .methods("GET"_method)(
            [&forumService, &commentService](const crow::request &req,
                                             int forumId) -> crow::response
            {
                std::string errorParam =
                    req.url_params.get("error") ? req.url_params.get("error") : "";
                std::string error;
    ...
    CROW_ROUTE(app, "/forum/<int>")
        .methods("POST"_method)(
                                             ...
                                             int forumId) -> crow::response
            {
                ...
                LOGINFO(username + " created a new comment/" + std::to_string(commentId) +
                        " on forum/" + std::to_string(forumId));
                return crow::response(200);
            });

    // Edit post
    CROW_ROUTE(app, "/forum/<int>/edit")
        .methods("POST"_method)(
            [&forumService](const crow::request &req, int forumId)
            {
                ...
                std::string error;
                if (!forumService.updateForumById(forumId, username, updatedPost,
                                                  updatedDescription, error))
                {
                    LOGERROR(error);
                    return crow::response(400, error);
                }
    ...
    // Edit comments
    CROW_ROUTE(app, "/comment/<int>/edit")
        .methods("POST"_method)(
            [&commentService](const crow::request &req, int commentId)
            {
                ...
                Comment comment;
                if (!commentService.getCommentById(commentId, comment, error))
                {
                    LOGERROR(error);
                    return crow::response(400, error);
                }

                LOGINFO(username + " edited comment/" + std::to_string(commentId) + " on forum/" +
