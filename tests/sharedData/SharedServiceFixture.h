...
struct SharedServiceFixture {
    SharedServiceFixture() {
        ...
        if (!ok) std::cerr << "Failed to clear users: " << error << std::endl;
        REQUIRE(ok);
        error.clear();

        ok = forumService.clearForums(error);
        if (!ok) std::cerr << "Failed to clear forums: " << error << std::endl;
        REQUIRE(ok);
        error.clear();

        ok = commentService.clearComments(error);
        if (!ok) std::cerr << "Failed to clear comments: " << error << std::endl;
        REQUIRE(ok);
        error.clear();
    }
